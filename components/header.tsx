...
export default function Header() {
  ...
  /** 仅「首页」阶段使用加高顶栏与大 Logo；SPA 根路径下其它阶段用紧凑顶栏，避免遮挡正文 padding 不足 */
  const isHomeStage = currentStage === 'home'
  const isHomePage = isHomeStage || pathname === '/'
  ...
  const showLogo = isHomeStage && isAtTop && !isHovering && !isAboutPage && !isGalleryPage // 只在首页顶部且未悬停时显示logo
  ...
  return (
    <header
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
        isAboutPage || isGalleryPage
          ...
          : 'h-12 shadow-lg' // 写作流程等：紧凑顶栏，与 paddingTop 120px 对齐
      }`}
    ...
    >
      <div className="max-w-7xl mx-auto h-full px-3 sm:px-4 lg:px-6 flex items-center justify-between gap-2 min-w-0">
        {/* Logo or Title */}
        <div className="flex items-center h-full shrink-0 min-w-0 max-w-[40%] sm:max-w-none">
          <Link 
            href="/" 
            onClick={(e) => {
              e.preventDefault()
          ...
          >
            {isHomeStage && showLogo ? (
              ...
              />
               ...
               <nav className="flex min-w-0 shrink items-center justify-end gap-1 sm:gap-2 lg:gap-3">
                 <div className="flex items-center gap-1 sm:gap-2 lg:gap-3 min-w-0 flex-wrap justify-end">
                 {navItems.map((item) => {
                   // 检查是否激活
                  ...
                  const isActive = isHomePageActive || isAboutPageActive || isWritePageActive || isGalleryPageActive
                   return (
                     <Link
                       ...
                       className={`px-2 py-1.5 rounded-md font-semibold text-xs sm:px-3 sm:py-2 sm:text-sm lg:px-5 lg:py-2.5 lg:text-base transition-all duration-200 whitespace-nowrap ${
                         isGalleryPage
                           ? isActive
                ...
                {/* About us 下拉菜單：改為點擊開關，不因移出 header 立即消失 */}
                 <div className="relative">
                   <button
                     ...
                     className={`px-2 py-1.5 rounded-lg font-semibold text-xs sm:px-3 sm:py-2 sm:text-sm lg:px-5 lg:py-2.5 lg:text-base transition-all duration-200 whitespace-nowrap ${
                       isAboutPage
                         ? showBackground
                           ? "text-yellow-200"
                           : "text-black"
                ...
                {/* My Farm 文字按钮 - 登录后显示，点击进入农场 */}
                 {userInfo && (
                   <button
                    ...
                    className={`relative flex-shrink-0 ml-0.5 px-2 py-1.5 rounded-md font-semibold text-xs sm:ml-1 sm:px-3 sm:py-2 sm:text-sm lg:px-5 lg:py-2.5 lg:text-base transition-all duration-200 whitespace-nowrap ${
                      showBackground
                        ? "text-blue-100 hover:text-white"
                        : "text-black drop-shadow-sm hover:text-gray-700"
